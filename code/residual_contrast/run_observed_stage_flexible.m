...
% linear SVM weights, Haufe patterns, and a paired standardized margin
% statistic (PMZ) on held-out subjects.
%
% INPUTS
...
% External (file) canonical form in your project: [ROI x Time x Subjects x Conditions]
% Internal (used here):                           [Subjects x ROI x Time x Conditions]
...
for b = 1:nBoot
    while true
        ...
        oob_mask = true(S,1); oob_mask(boot_idx) = false;
...
PMZ_obs   = zeros(T, nCon, nBoot);                    % paired standardized margin
...
%% ---------- 4) Main loop ----------
% If memory/broadcast becomes an issue, uncomment Constant:
...
parfor b = 1:nBoot
    ...
    Yte = [ones(nTe,1); -ones(nTe,1)];

    TA_b   = zeros(T, nCon);
    PMZ_b  = zeros(T, nCon);      % paired standardized margin
    ...
    for k = 1:nCon
        ...
        for t = 1:T
            ...
            d = s(1:nTe) - s(nTe+1:end);               % matched held-out subjects
            PMZ_b(t,k)  = paired_standardized_margin(d);

            w_keep = mdl.Beta;
            w_full = zeros(Rg,1,'single'); w_full(keep) = single(w_keep);

            % Haufe (fast): a = (X' * (X*w)) / (n-1) in z-space, then .* SD_train
            v = XtrZ * w_keep;                           % [nTr2 x 1]
    ...
    W_obs(:,:,:,b)     = W_b;
    Haufe_obs(:,:,:,b) = H_b;
    Beta_obs(:, b, :, :) = permute(W_b, [2 3 1]);  % [T x 1 x Con x R]
end

%% ---------- 5) Aggregate ----------
TA_mean    = mean(TA_obs,   3, 'omitnan');      % [T x Con]
TA_se      = std( TA_obs,   0, 3, 'omitnan');   % bootstrap SD (= SE of statistic)
PMZ_mean   = mean(PMZ_obs,  3, 'omitnan');      % paired standardized margin
...
meta.comp2         = COMP2;
meta.metric_TA     = 'OOB sample-level accuracy; chance = 0.5';
meta.metric_PMZ    = 'paired standardized margin = mean(d)/std(d), d = s_A - s_B on matched held-out subjects; null = 0';
...
try p = gcp('nocreate'); meta.nWorkers = p.NumWorkers; catch, meta.nWorkers = NaN; end

save(outf, 'TA_obs','TA_se','TA_mean', ...
...
cands = perms(1:4); best=[]; bestscore=-inf;
for i=1:size(cands,1)
    p = cands(i,:);
    Xsz = sz(p);
    Ssz=Xsz(1); Rsz=Xsz(2); Tsz=Xsz(3); Csz=Xsz(4);
    ...
    if sc > bestscore, bestscore = sc; best = p; end
end
...
if any(nanmask(:))
    for j = 1:size(Xtr_f,2)
        if any(nanmask(:,j)), Xtr_f(nanmask(:,j),j) = mu_full(j); end
    end
end
...
if any(nanmask(:))
    for j = 1:size(Xte_f,2)
        if any(nanmask(:,j)), Xte_f(nanmask(:,j),j) = mu_full(j); end
    end
end
XteZ = (Xte_f(:,keepIdx) - mu_full(keepIdx)) ./ sd_full(keepIdx);
end

function z = paired_standardized_margin(d)
% Paired standardized margin on held-out matched subjects.
% Returns mean(d)/std(d), with null = 0.
d = d(:);
...
if ~isempty(lbls) && i<=numel(lbls), s = lbls{i}; else, s = sprintf('cond%d',i); end
end

function v = tryget(f)
try, v = feval(f); catch, v = []; end
end
